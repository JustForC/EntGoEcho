...
func (cq *CompanyQuery) sqlAll(ctx context.Context) ([]*Company, error) {
	...
	if query := cq.withEmployees; query != nil {
		...
		for i := range nodes {
			...
			nodes[i].Edges.Employees = []*Employee{}
		}
		query.withFKs = true
		query.Where(predicate.Employee(func(s *sql.Selector) {
			s.Where(sql.InValues(company.EmployeesColumn, fks...))
		}))
		...
		for _, n := range neighbors {
			...
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "company_employees" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "company_employees" returned %v for node %v`, *fk, n.ID)
			}
